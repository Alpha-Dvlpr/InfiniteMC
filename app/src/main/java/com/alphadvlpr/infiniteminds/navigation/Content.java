...
public class Content extends AppCompatActivity {
    ...
    private boolean notSecond = false, notThird = false, notFourth = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button mainDownloadButton = findViewById(R.id.downloadButton),
        ...
        ImageView image = findViewById(R.id.imageContent);
        ...
        CardView otherContainer = findViewById(R.id.otherLinksContainer),
                imageContainer = findViewById(R.id.imageContainer),
                contentContainer = findViewById(R.id.contentContainer),
                chipContainer = findViewById(R.id.chipContainer);
        ChipGroup cg = findViewById(R.id.chipCategories);
        LinearLayout imagesContainer = findViewById(R.id.otherImagesContainer);
        ...
        if(Objects.requireNonNull(imagesList).isEmpty()){
            ...
            if(imagesList.size() == 1){ imagesContainer.setVisibility(View.GONE); }
            else{
        ...
        if(categoriesCounter == 0){ chipContainer.setVisibility(View.GONE); }
        else{
            Chip aux;
        ...
        for(int i = 0; i < downloadURL.size(); i++){
            if(downloadURL.get(0).equals("")){ mainDownloadButton.setVisibility(View.GONE); }
            else{ mainDownloadButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) { startDownload(downloadURL.get(0)); }
                }); }

            if(downloadURL.get(1).equals("")){
                secondDownloadButton.setVisibility(View.GONE);
                notSecond = true;
            }else{ secondDownloadButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) { startDownload(downloadURL.get(1)); }
                }); }

            if(downloadURL.get(2).equals("")){
                thirdDownloadButton.setVisibility(View.GONE);
                notThird = true;
            }else{ thirdDownloadButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) { startDownload(downloadURL.get(2)); }
                }); }

            if(downloadURL.get(3).equals("")){
                fourthDownloadButton.setVisibility(View.GONE);
                notFourth = true;
            }else{ fourthDownloadButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) { startDownload(downloadURL.get(3)); }
                }); }
        }

        if(notSecond && notThird && notFourth){ otherContainer.setVisibility(View.GONE); }
    }
